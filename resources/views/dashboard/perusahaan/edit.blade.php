...
<div class="page-header mb-4">
    <h1 class="h3 fw-bold mb-0">Edit Perusahaan</h1>
    <p class="subtitle text-muted">Ubah data perusahaan yang sudah ada.</p>
</div>
...
<div class="card p-4" style="max-width: 800px;">
    <form action="{{ route('admin.perusahaan.update', $perusahaan->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="nama" name="nama" class="form-control" value="{{ old('nama', $perusahaan->nama) }}" required>
            @error('nama') <span class="text-danger">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label class="form-label" for="lokasi">Lokasi (Kota/Daerah) <span style="color:#dc2626;">*</span></label>
            <input type="text" id="lokasi" name="lokasi" class="form-control" value="{{ old('lokasi', $perusahaan->lokasi) }}" required>
            @error('lokasi') <span class="text-danger">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label class="form-label" for="tentang">Tentang Perusahaan</label>
            <textarea id="tentang" name="tentang" class="form-control" rows="4">{{ old('tentang', $perusahaan->tentang) }}</textarea>
        ...
        <div class="row" style="display: flex; gap: 20px;">
            <div class="form-group" style="flex: 1;">
                ...
                <input type="url" id="website" name="website" class="form-control" value="{{ old('website', $perusahaan->website) }}">
                @error('website') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="form-group" style="flex: 1;">
                <label class="form-label" for="email">Email Kontak</label>
                <input type="email" id="email" name="email" class="form-control" value="{{ old('email', $perusahaan->email) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" id="alamat" name="alamat" class="form-control" value="{{ old('alamat', $perusahaan->alamat) }}">
            @error('alamat') <span class="text-danger">{{ $message }}</span> @enderror
        </div>

        <div class="form-group" style="width: 200px;">
            <label class="form-label" for="jumlah_mahasiswa">Kuota / Jumlah Mahasiswa</label>
            <input type="number" id="jumlah_mahasiswa" name="jumlah_mahasiswa" class="form-control" value="{{ old('jumlah_mahasiswa', $perusahaan->jumlah_mahasiswa) }}" min="0">
